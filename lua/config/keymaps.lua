...
local function git_push(remote_name, force)
    require('states.data').network_active = true
    ...
    vim.system(command, { text = true, cwd = cwd }, function(obj)
        ...
        require('states.data').network_active = false
        require('lualine').refresh()
    end)
end
...
-- Move up and down with Tab and Shift+Tab in Oil / Quickfix buffers
...
vim.keymap.set('n', '<Leader>gg', function()
    ...
    vim.defer_fn(function()
        if vim.fn.search('Unstaged') ~= 0
            or vim.fn.search('Untracked') ~= 0
            or vim.fn.search('Staged') ~= 0
        then
